...
void AgeLimit_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void AgeLimit_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "AgeLimit";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "equalTo";
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[1]); // line circom 23
...
AgeLimit_2_create(1,0,ctx,"main",0);
AgeLimit_2_run(0,ctx);
}
